...
class DescriptorSetLayoutDef {
  ...
  private:
    ...
    layers::unordered_map<uint32_t, uint32_t> binding_to_index_map_;
    ...
    layers::unordered_map<uint32_t, uint32_t> binding_to_dynamic_array_idx_map_;
...
class DescriptorSet : public BASE_NODE {
  public:
    ...
    // are present
    typedef layers::unordered_set<uint32_t> TrackedBindings;
  ...
  private:
    ...
    typedef layers::unordered_map<CMD_BUFFER_STATE *, TrackedBindings> TrackedBindingMap;
    // Track the validation caching of bindings vs. the command buffer and draw state
    typedef layers::unordered_map<uint32_t, CMD_BUFFER_STATE::ImageLayoutUpdateCount> VersionedBindings;
    struct CachedValidation {
        ...
        layers::unordered_map<const PIPELINE_STATE *, VersionedBindings> image_samplers;  // Tested vs. changes to CB's ImageLayout
    };
    typedef layers::unordered_map<const CMD_BUFFER_STATE *, CachedValidation> CachedValidationMap;
