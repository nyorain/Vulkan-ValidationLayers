...
static layers::unordered_map<VkCommandBuffer, VkCommandPool> secondary_cb_map{};
...
void StatelessValidation::CommonPostCallRecordEnumeratePhysicalDevice(const VkPhysicalDevice *phys_devices, const int count) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (0 == physical_device_properties_map.count(phys_device)) {
            ...
            layers::unordered_set<std::string> dev_exts_enumerated{};
    ...
    if (pCreateInfos != nullptr) {
        for (uint32_t i = 0; i < createInfoCount; ++i) {
            ...
            if (!(active_shaders & VK_SHADER_STAGE_MESH_BIT_NV) && (pCreateInfos[i].pVertexInputState != nullptr)) {
                ...
                layers::unordered_set<uint32_t> vertex_bindings(vertex_input_state->vertexBindingDescriptionCount);
                ...
                layers::unordered_set<uint32_t> attribute_locations(vertex_input_state->vertexAttributeDescriptionCount);
