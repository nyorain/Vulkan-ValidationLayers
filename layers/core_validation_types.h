...
class BASE_NODE {
  public:
    ...
    layers::unordered_map<CMD_BUFFER_STATE *, int> cb_bindings;
...
struct COMMAND_POOL_STATE : public BASE_NODE {
    ...
    layers::unordered_set<VkCommandBuffer> commandBuffers;
};
...
struct DEVICE_MEMORY_STATE : public BASE_NODE {
    ...
    layers::unordered_set<VulkanTypedHandle> obj_bindings;  // objects bound to this memory
    // Convenience vectors of handles to speed up iterating over objects independently
    layers::unordered_set<VkImage> bound_images;
    layers::unordered_set<VkBuffer> bound_buffers;
    layers::unordered_set<VkAccelerationStructureNV> bound_acceleration_structures;
...
class BINDABLE : public BASE_NODE {
  public:
    ...
    layers::unordered_set<MEM_BINDING> sparse_bindings;
    // True if memory will be imported/exported from/to an Android Hardware Buffer
    bool external_ahb;
    bool unprotected;  // can't be used for protected memory

    layers::unordered_set<DEVICE_MEMORY_STATE *> bound_memory_set_;
    ...
    const layers::unordered_set<DEVICE_MEMORY_STATE *> &GetBoundMemory() const { return bound_memory_set_; }
};
...
class IMAGE_STATE : public BINDABLE {
  public:
    ...
    layers::unordered_set<VkImage> aliasing_images;
...
struct RENDER_PASS_STATE : public BASE_NODE {
    ...
    layers::unordered_map<uint32_t, bool> attachment_first_read;
...
class PIPELINE_STATE : public BASE_NODE {
  public:
    struct StageState {
        layers::unordered_set<uint32_t> accessible_ids;
    ...
    layers::unordered_map<uint32_t, BindingReqMap> active_slots;
    ...
    layers::unordered_set<uint32_t> fragmentShader_writable_output_location_list;
    ...
    layers::unordered_map<uint32_t, uint32_t> vertex_binding_to_index_map_;
    std::vector<VkPipelineColorBlendAttachmentState> attachments;
    layers::unordered_set<VkShaderStageFlagBits, hash_util::HashCombiner::WrappedHash<VkShaderStageFlagBits>>
...
template <typename TransferBarrier>
using QFOTransferBarrierSet = layers::unordered_set<TransferBarrier, QFOTransferBarrierHash<TransferBarrier>>;
...
using GlobalQFOTransferBarrierMap =
    layers::unordered_map<typename TransferBarrier::HandleType, QFOTransferBarrierSet<TransferBarrier>>;

// Submit queue uses the Scoreboard to track all release/acquire operations in a batch.
template <typename TransferBarrier>
using QFOTransferCBScoreboard =
    layers::unordered_map<TransferBarrier, const CMD_BUFFER_STATE *, QFOTransferBarrierHash<TransferBarrier>>;
...
typedef layers::unordered_map<VkImage, Optional<ImageSubresourceLayoutMap>> CommandBufferImageLayoutMap;
...
struct CMD_BUFFER_STATE : public BASE_NODE {
    ...
    layers::unordered_map<VkDescriptorSet, std::vector<CmdDrawDispatchInfo>> validate_descriptorsets_in_queuesubmit;
    ...
    layers::unordered_set<std::shared_ptr<FRAMEBUFFER_STATE>> framebuffers;
    ...
    layers::unordered_set<VkEvent> waitedEvents;
    std::vector<VkEvent> writeEventsBeforeWait;
    std::vector<VkEvent> events;
    layers::unordered_set<QueryObject> activeQueries;
    layers::unordered_set<QueryObject> startedQueries;
    layers::unordered_set<QueryObject> resetQueries;
    ...
    layers::unordered_set<CMD_BUFFER_STATE *> linkedCommandBuffers;
    ...
    std::vector<std::function<bool(const ValidationStateTracker *device_data, bool do_validate, VkQueryPool &firstPerfQueryPool,
        ...
        queryUpdates;
    layers::unordered_set<cvdescriptorset::DescriptorSet *> validated_descriptor_sets;
